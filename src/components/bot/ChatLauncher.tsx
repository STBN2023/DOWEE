...
export default function ChatLauncher({
    ...
    async (opts?: { ignoreAfternoonFlag?: boolean; ignoreDismissed?: boolean }) => {
      ...
      try {
        ...
        setMessages((m) => [
          ...
          {
            role: "assistant",
            content: "Il est l’heure de vérifier/valider votre planning du jour. Souhaitez‑vous le faire maintenant ?",
          },
        ]);
        onOpenChange?.(true);
      } catch {
  ...
  // Invite à la connexion — configurable: ignorer ou non 'Plus tard'
  useEffect(() => {
  ...
  // Test manuel (Admin → Bot) — toujours ignorer 'Plus tard' pour forcer l’ouverture de test
  useEffect(() => {
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            <Footer onSend={(text) => { const t = (text || "").trim(); if (t) handleSend(t); }} />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
