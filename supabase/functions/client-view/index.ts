...
serve(async (req) => {
  ...
  const [{ data: project, error: projErr }, { data: client, error: cliErr }] = await Promise.all([
    admin.from("projects").select("id, code, name, client_id, tariff_id, quote_amount, budget_conception, budget_crea, budget_dev, version").eq("id", body.project_id).maybeSingle(),
  ...
  if ((project as any).tariff_id) {
    const { data: t, error: tErr } = await admin.from("ref_tariffs").select("id, label, rate_conception, rate_crea, rate_dev").eq("id", (project as any).tariff_id).maybeSingle();
  ...
  const result = {
    project: {
      ...
      quote_amount: (project as any).quote_amount,
      budgets: {
        conception: (project as any).budget_conception,
        crea: (project as any).budget_crea,
        dev: (project as any).budget_dev,
      },
      tariff: tariff ? {
        id: (tariff as any).id,
        label: (tariff as any).label,
        rate_conception: (tariff as any).rate_conception,
        rate_crea: (tariff as any).rate_crea,
        rate_dev: (tariff as any).rate_dev,
      } : null,
    },
    sold: {
      total_ht: (project as any).quote_amount ?? null,
      by_section: {
        conception: (project as any).budget_conception ?? null,
        crea: (project as any).budget_crea ?? null,
        dev: (project as any).budget_dev ?? null,
      }
    },
    realized: {
